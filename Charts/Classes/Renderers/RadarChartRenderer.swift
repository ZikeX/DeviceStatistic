...
//                let formatter = dataSet.valueFormatter
                // for show byte in labels
                let formatter = dataSet.byteFormatter
                let text = formatter?.stringFromByteCount(Int64(e.value))
                
                ChartUtils.drawText(context: context, text: text!, point: CGPoint(x: p.x, y: p.y - yoffset - valueFont.lineHeight), align: .Center, attributes: [NSFontAttributeName: valueFont, NSForegroundColorAttributeName: valueTextColor])
            }
        }
    }
