...
//                let formatter = dataSet.valueFormatter
                ...
                for (var j = 0, count = positions.count; j < count; j++)
                {
                    ...
                    // for byte labels
                    let text = formatter!.stringFromByteCount(Int64(val))
                    
                    ChartUtils.drawText(context: context, text: text, point: CGPoint(x: positions[j].x, y: positions[j].y - CGFloat(valOffset) - valueFont.lineHeight), align: .Center, attributes: [NSFontAttributeName: valueFont, NSForegroundColorAttributeName: valueTextColor])
                }
            }
        }
    }
