...
public class ChartYAxis: ChartAxisBase
{
    ...
    // for labels byte
    public var byteFormatter: NSByteCountFormatter?
    
    /// the formatter used to customly format the y-labels
    internal var _defaultValueFormatter = NSNumberFormatter()
    // for byte label
    internal var _defaultByteFormatter = NSByteCountFormatter()
    ...
    public override init()
    {
        ...
        _defaultByteFormatter.countStyle = .Binary
        _defaultByteFormatter.allowedUnits = .UseAll
        self.yOffset = 0.0
    }
    ...
    {
        let label = getLongestByteLabel() as NSString // change from getLongestLabel
    ...
    // for byte version
    public override func getLongestByteLabel() -> String
    {
        var longest = ""
        
        for (var i = 0; i < entries.count; i++)
        {
            let text = getByteFormattedLabel(i)
    ...
    public func getByteFormattedLabel(index: Int) -> String {
        if (index < 0 || index >= entries.count)
        {
            return ""
        }
        return (byteFormatter ?? _defaultByteFormatter).stringFromByteCount(Int64(entries[index]))
    }
